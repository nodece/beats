...
func WriteEventsReporterV2Cond(f mb.ReportingMetricSetV2, t testing.TB, path string, cond func(mapstr.M) bool) error {
	if !*flags.DataFlag {
...
// the condition to a file.
func WriteEventsReporterV2WithContextCond(f mb.ReportingMetricSetV2WithContext, t testing.TB, path string, cond func(mapstr.M) bool) error {
	if !*flags.DataFlag {
		t.Skip("skip data generation tests")
	}
...
func writeEvent(events []mb.Event, f mb.MetricSet, t testing.TB, path string, cond func(mapstr.M) bool) error {
	if len(events) == 0 {
...
func CreateFullEvent(ms mb.MetricSet, metricSetData mapstr.M) beat.Event {
...
func SelectEvent(events []mapstr.M, cond func(e mapstr.M) bool) (mapstr.M, error) {
...
func SelectEventV2(f mb.MetricSet, events []mb.Event, cond func(e mapstr.M) bool) (mb.Event, error) {
	if cond == nil && len(events) > 0 {
		return events[0], nil
	}
	for _, e := range events {
